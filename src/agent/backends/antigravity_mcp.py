...
def parse_args():
    ...
    parser.add_argument("--token", type=str, default="", help="Auth token of the loopback bridge server")
    return parser.parse_args()


async def run_server(port: int, token: str):
    app = Server("slon")
    base_url = f"http://127.0.0.1:{port}"
    headers = {"X-Slon-Token": token}
    ...
    async def call_tool(name: str, arguments: dict) -> list[types.TextContent]:
        try:
            async with httpx.AsyncClient(timeout=120.0, trust_env=False, headers=headers) as client:
...
def main():
    args = parse_args()
    asyncio.run(run_server(args.port, args.token))
