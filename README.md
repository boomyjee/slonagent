...
После этого папки появляются как `/mnt/ro/c/users/me/downloads` и `/mnt/rw/d/projects/my-app`. Под капотом — junctions на Windows / `mount --bind` на Linux: контейнер вообще не перезапускается на изменение списка папок.
