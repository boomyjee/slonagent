...
Внутри каждого агента (основного или форка) может быть несколько **тредов** — независимых потоков диалога с общей памятью.

Каждый тред — это отдельный поток диалога (`CONTEXT_{uuid}.json`) со своей историей сообщений. Системный промпт (субличности), факты и observations **общие** для всех тредов агента — но observations помечены идентификатором треда, так что агент знает, в каком треде что произошло.

```
memory/
  CONTEXT.json            ← главный тред (uuid = "")
  CONTEXT_c84b0ee8.json   ← второй тред
  THREADS.json            ← реестр: uuid → имя
  fact/                   ← общая для всех тредов
  personality/            ← общая для всех тредов
```

В веб-интерфейсе треды переключаются через вкладки. В Telegram — через топики группы.
...
```
memory/
  CONTEXT.json            ← главный тред: история + compressed observations
  CONTEXT_{uuid}.json     ← дополнительные треды
  THREADS.json            ← реестр тредов: uuid → имя
  CRON.json               ← задачи планировщика
  LOG.md                  ← debug-дамп наблюдений (только для чтения)
  fact/
    facts.db              ← SQLite: факты, сущности, observations (общие для тредов)
    lancedb/              ← векторный индекс
  personality/            ← субличности (.md файлы, общие для тредов)
  workspace/              ← рабочая директория агента (файлы, артефакты)
forks/
  <agent_id>/             ← форк-агент со своей memory/ и .config.json
```
